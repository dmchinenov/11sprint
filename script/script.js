(function() {
  ...
  const popupAddCard = new Popup(document.querySelector('#cardpopup'), 'popup_is-opened');
  const popupOpenProfile = new Popup(document.querySelector('#editpopup'), 'popup_is-opened');
  const popupOpenPhoto = new Popup(document.querySelector('#photopopup'), 'popup_is-opened');
  ...
  formNewCard.addEventListener('submit', function(event) {
    ...
    if (validateFormCard.isFormValid(formNewCard)) {
      api.addCard(formNewCard.name.value, formNewCard.link.value).then(res => {
  ...
  formEditProfile.addEventListener('submit', function(event) {
    ...
    if (validateFormUser.isFormValid(formEditProfile)) {
      api.setUserInfo(formname.value, formjob.value).then(res => {
          userInfo.updateUserInfo(res.name, res.about, res.avatar)
